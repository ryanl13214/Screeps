     ...
     run: function(creep)
     {
         ...
         if(check)
         {
             ...
             if(creep.memory.memstruct.full == false)
             {
                 ...
                 if(droppedresources == undefined && creep.memory.target == "a")
                 {
                     ...
                     containers = creep.pos.findClosestByRange(FIND_STRUCTURES,
                     {
                         filter: (structure) =>
                         {
                             return ((structure.structureType == STRUCTURE_CONTAINER && structure.store.energy > 1500) || (structure.structureType == STRUCTURE_LINK && structure.store.energy > 700));
                         }
                     });
             ...
             if(creep.memory.memstruct.full == true)
             {
                 ...
                 {
                     if(creep.room.controller != undefined && creep.room.controller.level < 8 && creep.room.storage != undefined)
                     {
