...
var rolescout = {
    run: function(creep)
    {
        if (creep.room.name == creep.memory.prevRoom)
        {
            ...
            {
                creep.memory.exitchosen = (creep.memory.exitchosen + 1) % roomnames.length;
            }
        ...
        else
        {
            if (creep.memory.exitchosen != "a" && creep.room.name != creep.memory.prevRoom)
            {
                ...
                else
                {
                    ...
                    if (Game.flags[creep.memory.home].memory.extraMineRooms == undefined || Game.flags[creep.memory.home].memory.extraMineRooms == "a" && creep.ticksToLive > 1400)
                    {
